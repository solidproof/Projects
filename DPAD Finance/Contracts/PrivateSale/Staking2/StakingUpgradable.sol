...
contract StakingUpgradable is OwnableUpgradeable {
    ...
    function initialize(IBEP20 _DPAD,
        uint256 _DPADPerBlock,
        uint256 _startBlock,
        IStaking _oDpadStaking) public initializer {
...
//        require (_pid != 0, 'deposit DPAD by staking');
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                // this could be removed to prevent withdraw type harvesting
                // and could be just added this amount to user's amount
                safeDpadTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
//            pool.stakingToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            IStaking.UserInfo memory _user = oDpadStaking.userInfo(_pid, msg.sender);
            require(_user.amount > user.amount.add(_amount), "wut?");

            user.amount = _user.amount;
        }
...
//        require (_pid != 0, 'withdraw DPAD by unstaking');
        ...
        // Sync amounts first, for just in case
        user.amount = oDpadStaking.userInfo(_pid, msg.sender).amount;

        // withdraw from original staking
        oDpadStaking.withdraw(_pid, _amount);

        // Then proceed to normal process
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accDpadPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0) {
            safeDpadTransfer(msg.sender, pending);
    ...
    // Carefull, for already existing users, it'll wipe out their rewards, so do not add already added users
    function addUsersFromODapdStaking(uint _pid, address[] memory _addresses) onlyOwner public {
        for (uint i = 0; i < _addresses.length; i++) {
            UserInfo storage user = userInfo[_pid][_addresses[i]];
            IStaking.UserInfo memory _user = oDpadStaking.userInfo(_pid, _addresses[i]);
            user.amount = _user.amount;
            user.lockedTill = _user.lockedTill;
            user.rewardDebt = 0; // don't copy reward debt, as it'll start as new in this contract
        }
    }
