...
contract HooliesNFT is ERC721AQueryable, DefaultOperatorFilterer, Ownable, ReentrancyGuard {
  ...
  function setMintStartTimestamp(uint _value) public onlyOwner {
    require(_value <= block.timestamp, "mintStartTimestamp cannot be less than blockTimestamp");
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount)  {
    ...
    require(wlClaimed[msg.sender] + _mintAmount <= wlLimit, "max limit for wallet reached");
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount)  {
    ...
    require(publicClaimed[msg.sender] + _mintAmount <= publicLimit, "max limit for wallet reached");
