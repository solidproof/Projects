...
/*
 * This contract is a xESPER Usage (plugin) that can boost spNFTs' yield (staking positions on NFTPools) when it
 * receives allocations from the xESPERToken contract
 */
contract YieldBooster is Ownable, ReentrancyGuard, IXEsperTokenUsage, IYieldBooster {
  ...
  // User's position xESPER total allocation
  ...
  mapping(address => uint256) private _usersTotalAllocation; // User's xESPER total allocation
  ...
  /**
   * @dev Returns total xESPER allocated to this contract by "poolAddress"
   */
  ...
  /**
   * @dev Returns allocated xESPER to "tokenId" spNFT from "poolAddress" NFTPool by "userAddress"
   */
  ...
  /**
   * Allocates "userAddress" user's "amount" of xESPER to this usage contract
   * "data" should contain tokenId and poolAddress
   *
   * Can only be called by XEsperToken contract, which is trusted to verify amounts
   */
  function allocate(address userAddress, uint256 amount, bytes calldata data) external override nonReentrant xEsperTokenOnly {
    (address poolAddress, uint256 tokenId) = abi.decode(data, (address, uint256));
    _allocate(userAddress, poolAddress, tokenId, amount);

    // allocated xESPER is added (as boost points) to spNFT
    INFTPool(poolAddress).boost(tokenId, amount);
  }

  /**
   * Deallocates "userAddress" user's "amount" of xESPER from this usage contract
   * "data" should contain tokenId and poolAddress
   *
   * Can only be called by XEsperToken contract, which is trusted to verify amounts
   */
  function deallocate(address userAddress, uint256 amount, bytes calldata data) external override nonReentrant xEsperTokenOnly {
    (address poolAddress, uint256 tokenId) = abi.decode(data, (address, uint256));
    _deallocate(userAddress, poolAddress, tokenId, amount);

    // should only be called if spNFT has not been burned, to avoid having stuck xESPER on it
    if(INFTPool(poolAddress).exists(tokenId)) {
      // allocated xESPER is removed (as boost points) from the spNFT
   ...
   * The pool should remove the allocated xESPER (boost points) from its own spNFT when calling this function
   */
  function deallocateAllFromPool(address userAddress, uint256 tokenId) external override nonReentrant {
    ...
    // update user's xESPER allocations balance
    xEsperToken.deallocateFromUsage(userAddress, amount);
  }

  /**
   * Deallocates msg.sender's "amount" of xESPER from XEsperToken, without adjusting this contract allocations balances
   *
  ...
  function forceDeallocate() external nonReentrant {
    ...
    uint256 amount = xEsperToken.usageAllocations(msg.sender, address(this));

    // update user's xESPER allocations balance
    xEsperToken.deallocateFromUsage(msg.sender, amount);
  }
  ...
  /**
   * @dev Returns multiplier that should be applied to a spNFT based on its boost points (allocated xESPER)
   *
   ...
   * 1% of the pool's allocated xESPER
   *
  ...
  /**
   * @dev Allocates "userAddress" user's "amount" of xESPER to "tokenId" spNFT of "poolAddress" pool
   */
  ...
  /**
   * @dev Deallocates "userAddress" user's "amount" of xESPER allocated to "tokenId" spNFT of "poolAddress" pool
   */
  function _deallocate(address userAddress, address poolAddress, uint256 tokenId, uint256 amount) internal {
    ...
    require(userPositionAllocation >= amount, "deallocate: not enough allocated xEsper");
