...
/*
 * This contract is used to distribute dividends to users that allocated xEsper here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xEsper allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract Dividends is Ownable, ReentrancyGuard, IXEsperTokenUsage, IDividends {
  ...
  address public immutable xEsperToken; // xEsperToken contract

  mapping(address => uint256) public usersAllocation; // User's xEsper allocation
  ...
  constructor(address xEsperToken_, uint256 startTime_) {
    require(xEsperToken_ != address(0), "zero address");
    xEsperToken = xEsperToken_;
  ...
  modifier xEsperTokenOnly() {
    require(msg.sender == xEsperToken, "xEsperTokenOnly: caller should be XEsperToken");
    _;
  }
  ...
  /**
   * Allocates "userAddress" user's "amount" of xEsper to this dividends contract
   *
  ...
  /**
   * Deallocates "userAddress" user's "amount" of xEsper allocation from this dividends contract
   *
   * Can only be called by xEsperToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IxEsperTokenUsage)
   */
  ...
  */
  ...
  function _updateDividendsInfo(address token) internal {
    ...
    // if no xEsper is allocated or initial distribution has not started yet
  ...
  function _harvestDividends(address token) internal {
    ...
    uint256 userXEsperAllocation = usersAllocation[msg.sender];
    uint256 pending = user.pendingDividends.add(
      userXEsperAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
    );

    user.pendingDividends = 0;
    user.rewardDebt = userXEsperAllocation.mul(accDividendsPerShare).div(1e18);
