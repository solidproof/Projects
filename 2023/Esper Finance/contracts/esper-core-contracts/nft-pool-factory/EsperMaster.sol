...
/*
 * This contract centralizes Esper's yield incentives distribution.
 * Pools that should receive those incentives are defined here, along with their allocation.
 * All rewards are claimed from the ESPERToken contract.
 */
contract EsperMaster is Ownable, IEsperMaster {
  ...
  IEsperToken private immutable _esperToken; // Address of the ESPER token contract
  ...
  constructor(
    IEsperToken esperToken_,
    uint256 startTime_
  ) {
    require(address(esperToken_) != address(0), "EsperMaster: esperToken cannot be set to zero address");
    require(_currentBlockTimestamp() < startTime_ && startTime_ >= esperToken_.lastEmissionTime(), "EsperMaster: invalid startTime");
    _esperToken = esperToken_;
    startTime = startTime_; // Must be set with the same time as EsperToken emission start
  }
  ...
  /*
   * @dev Returns EsperToken address
   */
  function esperToken() external view override returns (address) {
    return address(_esperToken);
  }

  /*
   * @dev Returns EsperToken's emission rate (allocated to this contract)
   */
  function emissionRate() public view returns (uint256) {
    return _esperToken.masterEmissionRate();
  }

  /**
   * @dev Returns current owner's address
   */
  function owner() public view virtual override(IEsperMaster, Ownable) returns (address) {
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256 effectiveAmount) {
    uint256 esperBalance = _esperToken.balanceOf(address(this));
  ...
  function _updatePool(address poolAddress) internal {
    ...
    if (allocPoint > 0 && INFTPool(poolAddress).hasDeposits()) {
      // calculate how much ESPER rewards are expected to be received for this pool
      ...
      (rewards) = _esperToken.claimMasterRewards(rewards);
