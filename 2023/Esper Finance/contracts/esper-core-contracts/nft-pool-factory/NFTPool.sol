 ...
 * spNFTs are yield-generating positions when the NFTPool contract has allocations from the Esper Master
 */
contract NFTPool is ReentrancyGuard, INFTPool, ERC721("Esper staking position NFT", "spNFT") {
  ...
  struct StakingPosition {
    ...
    uint256 boostPoints; // Allocated xESPER from yieldboost contract (optional)
    uint256 totalMultiplier; // lockMultiplier + allocated xESPER boostPoints multiplier
    uint256 pendingXEsperRewards; // Not harvested xEsper rewards
    uint256 pendingEsperRewards; // Not harvested Esper rewards
  }
  ...
  IEsperMaster public master; // Address of the master
  ...
  IERC20 private _esperToken; // EsperToken contract's address
  IXEsperToken private _xEsperToken; // XEsperToken contract's address
  ...
  uint256 private _maxBoostMultiplier = 10000; // 100%, Max boost that can be earned from xEsper yieldBooster

  uint256 private constant _TOTAL_REWARDS_SHARES = 10000; // 100%, high limit for xEsperRewardsShare
  uint256 public xEsperRewardsShare = 8000; // 80%, directly defines esperShare with the remaining value to 100%
  ...
  function initialize(IEsperMaster master_, IERC20 esperToken, IXEsperToken xEsperToken, IERC20 lpToken) external {
    require(msg.sender == factory && !initialized, "FORBIDDEN");
    _lpToken = lpToken;
    master = master_;
    _esperToken = esperToken;
    _xEsperToken = xEsperToken;
    initialized = true;

    // to convert ESPER to xESPER
   _esperToken.approve(address(_xEsperToken), type(uint256).max);
  }
  ...
  event SetXEsperRewardsShare(uint256 xEsperRewardsShare);
  ...
  function getPoolInfo() external view override returns (
    address lpToken, address esperToken, address xEsperToken, uint256 lastRewardTime, uint256 accRewardsPerShare,
    ...
    return (
    address(_lpToken), address(_esperToken), address(_xEsperToken), lastRewardTime, _accRewardsPerShare,
  ...
  function pendingRewards(uint256 tokenId) external view returns (uint256) {
    ...
    return position.amountWithMultiplier.mul(accRewardsPerShare).div(1e18).sub(position.rewardDebt)
      .add(position.pendingXEsperRewards).add(position.pendingEsperRewards);
  }
  ...
  /**
   * @dev Set the share of xESPER for the distributed rewards
   * The share of ESPER will incidently be 100% - xEsperRewardsShare
   *
   * Must only be called by the owner
   */
  function setXEsperRewardsShare(uint256 xEsperRewardsShare_) external {
    _requireOnlyOwner();
    require(xEsperRewardsShare_ <= _TOTAL_REWARDS_SHARES, "too high");

    xEsperRewardsShare = xEsperRewardsShare_;
    emit SetXEsperRewardsShare(xEsperRewardsShare_);
  }
  ...
  function createPosition(uint256 amount, uint256 lockDuration) external nonReentrant {
    ...
    _stakingPositions[currentTokenId] = StakingPosition({
      ...
      pendingEsperRewards: 0,
      pendingXEsperRewards: 0
    });
   ...
   * Users should still be able to deallocate xESPER from the YieldBooster contract
   */
  ...
  function _harvestPosition(uint256 tokenId, address to) internal {
    ...
    // transfer rewards
    if (pending > 0 || position.pendingXEsperRewards > 0 || position.pendingEsperRewards > 0) {
      uint256 xEsperRewards = pending.mul(xEsperRewardsShare).div(_TOTAL_REWARDS_SHARES);
      uint256 esperAmount = pending.add(position.pendingEsperRewards).sub(xEsperRewards);

      xEsperRewards = xEsperRewards.add(position.pendingXEsperRewards);

      // Stack rewards in a buffer if to is equal to address(0)
      if (address(0) == to) {
        position.pendingXEsperRewards = xEsperRewards;
        position.pendingEsperRewards = esperAmount;
      }
      else {
        // convert and send xESPER + ESPER rewards
        position.pendingXEsperRewards = 0;
        position.pendingEsperRewards = 0;

        if(xEsperRewards > 0) xEsperRewards = _safeConvertTo(to, xEsperRewards);
        // send share of ESPER rewards
        esperAmount= _safeRewardsTransfer(to, esperAmount);

        // forbidden to harvest if contract has not explicitly confirmed it handle it
        _checkOnNFTHarvest(to, tokenId, esperAmount, xEsperRewards);
      }
    }
  ...
  */
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256) {
    uint256 balance = _esperToken.balanceOf(address(this));
    // cap to available balance
    if (amount > balance) {
      amount = balance;
    }
    _esperToken.safeTransfer(to, amount);
    return amount;
  }

  /**
   * @dev Safe convert ESPER to xESPER function, in case rounding error causes pool to not have enough tokens
   */
  function _safeConvertTo(address to, uint256 amount) internal returns (uint256) {
    ...
    if(amount > 0 ) _xEsperToken.convertTo(amount, to);
    return amount;
  }
   ...
   */
  function _checkOnNFTHarvest(address to, uint256 tokenId, uint256 esperAmount, uint256 xEsperAmount) internal {
    address nftOwner = ERC721.ownerOf(tokenId);
    if (nftOwner.isContract()) {
      bytes memory returndata = nftOwner.functionCall(abi.encodeWithSelector(
          INFTHandler(nftOwner).onNFTHarvest.selector, msg.sender, to, tokenId, esperAmount, xEsperAmount), "non implemented");
