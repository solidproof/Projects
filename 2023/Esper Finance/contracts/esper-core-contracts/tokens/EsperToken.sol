...
/*
 * ESPER is Esper's native ERC20 token.
 * It has an hard cap and manages its own emissions and allocations.
 */
contract EsperToken is Ownable, ERC20("Esper token", "ESPER"), IEsperToken {
   ...
   */
  modifier onlyMaster() {
    require(msg.sender == masterAddress, "EsperToken: caller is not the master");
    _;
  }
  ...
  function treasuryAllocation() public view returns (uint256) {
    return uint256(ALLOCATION_PRECISION).sub(farmingAllocation);
  }
  ...
  function emitAllocations() public {
    ...
    uint256 masterShare = newEmissions.mul(farmingAllocation).div(ALLOCATION_PRECISION);
  ...
  /**
   * @dev Burns "amount" of ESPER by sending it to BURN_ADDRESS
   */
  ...
  function updateAllocations(uint256 farmingAllocation_) external onlyOwner {
    ...
    require(farmingAllocation_ <= 100, "updateAllocations: total allocation is too high");
  ...
  /**
   * @dev Updates ESPER emission rate per second
   *
   * Must only be called by the owner
   */
