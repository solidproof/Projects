...
contract RushB is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20("RushB Hub", "RushB") {
        buyBackWallet = 0xE9830Cae536e492f39138f5f963369B49535E7f2; // set as buy back wallet
        uint256 _buyLiquidityFee = 0;
        uint256 _buyBuyBackFee = 300;

        uint256 _sellLiquidityFee = 0;
        uint256 _sellBuyBackFee = 500;

        uint256 totalSupply = 128_000_000 * 1e18;

        swapTokensAtAmount = (totalSupply * 1) / 10000; // 0.01% swap wallet

        buyLiquidityFee = _buyLiquidityFee;
        buyBuyBackFee = _buyBuyBackFee;
        buyTotalFees = buyLiquidityFee + buyBuyBackFee;

        sellLiquidityFee = _sellLiquidityFee;
        sellBuyBackFee = _sellBuyBackFee;
        sellTotalFees = sellLiquidityFee + sellBuyBackFee;
    ...
    function clearStuckNativeTokens() public onlyOwner {
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0) {
                ...
                tokensForBack += (fees * sellBuyBackFee) / sellTotalFees;
            }
            // on buy
            else if (automatedMarketMakerPairs[from] && buyTotalFees > 0) {
                fees = amount.mul(buyTotalFees).div(10000);
                tokensForLiquidity += (fees * buyLiquidityFee) / buyTotalFees;
                tokensForBack += (fees * buyBuyBackFee) / buyTotalFees;
            }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForBack;
        bool success;
        ...
        uint256 ethForBack = ethBalance.mul(tokensForBack).div(
            totalTokensToSwap
        );

        uint256 ethForLiquidity = ethBalance - ethForBack;

        tokensForLiquidity = 0;
        tokensForBack = 0;

        (success, ) = address(buyBackWallet).call{value: ethForBack}("");
