...
contract GovernorBravoEvents {
    ...
    event QuorumVotesSet(uint quorumVotes, uint newquorumVotes);
    ...
    /// @notice Emitted when Token governance token is updated.
    event GovernanceTokenSet(address oldToken, address newToken);

    /// @notice Emitted when Timelock is updated
    event TimeAddressSet(address oldTimelock, address newTimelock);
...
contract GovernorBravoDelegateStorageV1 is GovernorBravoDelegatorStorage {
    ...
    /// @notice The address of the token Protocol Timelock
    TimelockInterface public timelock;

    /// @notice The address of the token governance token
