...
import {veXNF} from "../veXNF.sol";

/*
 * @title veXNFFactory
 *
 * @notice A factory contract for deploying veXNF instances.
 *
 * Co-Founders:
 ...
 */
contract veXNFFactory {

    /// --------------------------------- EXTERNAL FUNCTION --------------------------------- \\\

    /**
     * @notice Deploys a new instance of the veXNF contract.
     * @return The address of the newly deployed veXNF contract.
     */
    function deployveXNF() external returns (address) {
        veXNF vexnf = new veXNF();
        return address(vexnf);
    }
