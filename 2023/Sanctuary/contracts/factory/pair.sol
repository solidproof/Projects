...
contract SanctuaryPair is IPair, UniswapV2ERC20 {
  ...
  uint16 public token0FeePercent = 250; // default = 0.25%  // uses single storage slot, accessible via getReserves
  uint16 public token1FeePercent = 250; // default = 0.25%  // uses single storage slot, accessible via getReserves
  ...
  modifier lock() {
    require(unlocked == 1, 'SanctuaryPair: LOCKED');
    unlocked = 0;
    _;
    unlocked = 1;
  }
  ...
  function _safeTransfer(address token, address to, uint value) private {
    ...
    require(success && (data.length == 0 || abi.decode(data, (bool))), 'SanctuaryPair: TRANSFER_FAILED');
  }
  ...
  function initialize(address _token0, address _token1) external {
    require(msg.sender == factory && !initialized, 'SanctuaryPair: FORBIDDEN');
    // sufficient check
    token0 = _token0;
    token1 = _token1;
  ...
  */
  function setFeePercent(uint16 newToken0FeePercent, uint16 newToken1FeePercent) external lock {
    require(msg.sender == IFactory(factory).feePercentOwner(), "SanctuaryPair: only factory's feeAmountOwner");
    require(newToken0FeePercent <= MAX_FEE_PERCENT && newToken1FeePercent <= MAX_FEE_PERCENT, "SanctuaryPair: feePercent mustn't exceed the maximum");
    require(newToken0FeePercent > 0 && newToken1FeePercent > 0, "SanctuaryPair: feePercent mustn't exceed the minimum");
  ...
  function setStableSwap(bool stable, uint112 expectedReserve0, uint112 expectedReserve1) external lock {
    require(msg.sender == IFactory(factory).setStableOwner(), "SanctuaryPair: only factory's setStableOwner");
    require(!pairTypeImmutable, "SanctuaryPair: immutable");

    require(stable != stableSwap, "SanctuaryPair: no update");
    require(expectedReserve0 == reserve0 && expectedReserve1 == reserve1, "SanctuaryPair: failed");
  ...
  function setPairTypeImmutable() external lock {
    require(msg.sender == IFactory(factory).owner(), "SanctuaryPair: only factory's owner");
    require(!pairTypeImmutable, "SanctuaryPair: already immutable");
  ...
  function _update(uint balance0, uint balance1) private {
    require(balance0 <= uint112(- 1) && balance1 <= uint112(- 1), 'SanctuaryPair: OVERFLOW');
  ...
  function mint(address to) external lock returns (uint liquidity) {
    ...
    require(liquidity > 0, 'SanctuaryPair: INSUFFICIENT_LIQUIDITY_MINTED');
    _mint(to, liquidity);
  ...
  function burn(address to) external lock returns (uint amount0, uint amount1) {
    ...
    require(amount0 > 0 && amount1 > 0, 'SanctuaryPair: INSUFFICIENT_LIQUIDITY_BURNED');
  ...
  function _swap(TokensData memory tokensData, address to, bytes memory data, address referrer) internal lock {
    require(tokensData.amount0Out > 0 || tokensData.amount1Out > 0, 'SanctuaryPair: INSUFFICIENT_OUTPUT_AMOUNT');

    (uint112 _reserve0, uint112 _reserve1, uint16 _token0FeePercent, uint16 _token1FeePercent) = getReserves();
    require(tokensData.amount0Out < _reserve0 && tokensData.amount1Out < _reserve1, 'SanctuaryPair: INSUFFICIENT_LIQUIDITY');


    {
      require(to != tokensData.token0 && to != tokensData.token1, 'SanctuaryPair: INVALID_TO');
    ...
    require(amount0In > 0 || amount1In > 0, 'SanctuaryPair: INSUFFICIENT_INPUT_AMOUNT');
    ...
    {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
      ...
      require(_k(balance0Adjusted, balance1Adjusted) >= _k(uint(_reserve0), uint(_reserve1)), 'SanctuaryPair: K');
    }
  ...
  function sync() external lock {
    ...
    require(token0Balance != 0 && token1Balance != 0, "SanctuaryPair: liquidity ratio not initialized");
  ...
  */
  function drainWrongToken(address token, address to) external lock {
    require(msg.sender == IFactory(factory).owner(), "SanctuaryPair: only factory's owner");
    require(token != token0 && token != token1, "SanctuaryPair: invalid token");
