...
contract Vester is ReentrancyGuardUpgradeable, OwnableUpgradeable, IVester {
        ...
        if (hasVolumeLimit) {
            return
                maxAmount > historicalCumulativeClaimAmounts[_account]
                    ? maxAmount -
                        historicalCumulativeClaimAmounts[_account] -
                        _getNextClaimableAmount(_account)
                    : 0;
        } else {
            return maxAmount;
        }
    ...
    function _updateVesting(address _account) private {
        ...
        historicalCumulativeClaimAmounts[_account] = historicalCumulativeClaimAmounts[_account].add(
            amount
        );
