...
contract LiquidityGeneratorToken is IERC20, OwnableUpgradeable, BaseToken {
    ...
    function initialize(
        ...
        require(feeSettings_[0] >= 0, "Invalid tax fee");
        require(feeSettings_[1] >= 0, "Invalid liquidity fee");
        require(feeSettings_[2] >= 0, "Invalid charity fee");
        require(feeSettings_[3] >= 0, "Invalid marketing fee");
        ...
        require(
            feeSettings_[0] + feeSettings_[1] + feeSettings_[2] + feeSettings_[3] <= 10**4 / 4,
        ...
        _taxFee = feeSettings_[0];
        _previousTaxFee = _taxFee;

        _liquidityFee = feeSettings_[1];
        _previousLiquidityFee = _liquidityFee;

        _charityAddress = addrs[1];
        _charityFee = feeSettings_[2];
        _previousCharityFee = _charityFee;

        _marketingAddress = addrs[2];
        _marketingFee = feeSettings_[3];
        ...
        _isExcludedFromFee[poolFactory_] = true;  // For TokenFactory
    ...
    function _transferBothExcluded(
        ...
        _takeLiquidity(tFees[1]);
        _takeCharityFee(tFees[2]);
        _takeMarketingFee(tFees[3]);
        _reflectFee(rFee, tFees[0]);
    ...
    function setTaxFeePercent(uint256 taxFeeBps) external onlyOwner {
        _taxFee = taxFeeBps;
        require(
            _taxFee + _liquidityFee + _charityFee + _marketingFee <= 10**4 / 4,
            "Total fee is over 25%"
        );
    }
        ...
        )
    {
        ...
        tFees[3] = tMarketingFee;
        tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tCharityFee).sub(tMarketingFee);
        ...
        )
    {
        ...
        uint256 rFee = tFees[0].mul(currentRate);
        uint256 rLiquidity = tFees[1].mul(currentRate);
        uint256 rCharity = tFees[2].mul(currentRate);
        uint256 rMarketing = tFees[3].mul(currentRate);
