...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    /**
     * @dev Spend `amount` form the allowance of `_owner` toward `spender`.
     *
 ...
 * there is an account (an _owner) that can be granted exclusive access to
 * specific functions.
 *
 * By default, the _owner account will be the one that deploys the contract. This
...
abstract contract Ownable is Context {
    ...
    /**
     * @dev Initializes the contract setting the deployer as the initial _owner.
     */
    constructor() {
    ...
    /**
     * @dev Returns the address of the current _owner.
     */
    ...
    /**
     * @dev Throws if called by any account other than the _owner.
     */
    modifier onlyOwner() {
        require(Owner() == _msgSender(), "Ownable: caller is not the _owner");
        _;
    }

    /**
     * @dev Leaves the contract without _owner. It will not be possible to call
     * `onlyOwner` functions anymore. Can only be called by the current _owner.
     *
     * NOTE: Renouncing ownership will leave the contract without an _owner,
     * thereby removing any functionality that is only available to the _owner.
     */
     ...
     * Can only be called by the current _owner.
     */
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "Ownable: new _owner is the zero address");
...
contract PZToken is ERC20, Ownable {
    ...
    function setOwner(address owner_) external onlyOwner {
        require(owner_ != address(0),"Invalid");
        _owner = owner_;
    }
    ...
    function transferERC20 (IERC20 token,address _to, uint256 amount) external onlyOwner
    {
        ...
        /////require(msg.value == amount,"Amount should be equal");
        amount *= 10 ** decimals();
