...
contract GOV is ERC20Burnable, Ownable {
    ...
    function addMasterChef(address _MC) external {
        ...
        MasterChef = _MC;
        allowedMinters[_MC] = true;
    }

    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).

    function mint(address to, uint256 amount) external {
        require(allowedMinters[_msgSender()], "Minter not allowed");
        _mint(to, amount);
        ...
        returns (bool)
    {
        ...
        require(
            owner == MasterChef,
            "Cant transfer GOV tokens from account to account"
        );
