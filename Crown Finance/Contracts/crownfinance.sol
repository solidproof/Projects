...
contract Token is Ownable, ERC20 {
    ...
    function initialize(string memory name_, string memory symbol_, address _airdrop) external initializer {
        Ownable.__Ownable_init();
        ERC20.__ERC20_init(name_, symbol_);

        teamLockToken = new TokenVesting(msg.sender, block.timestamp + 180 days, 0 days, 150 days);
        adLockToken = new TokenVesting(msg.sender, block.timestamp + 90 days, 0 days, 300 days);

        __mint(msg.sender, 10 * 1e6 * 1e18);
        __mint(_airdrop, 15 * 1e6 * 1e18);
        __mint(address(teamLockToken), 5 * 1e6 * 1e18);
        __mint(address(adLockToken), 10 * 1e6 * 1e18);
    }
