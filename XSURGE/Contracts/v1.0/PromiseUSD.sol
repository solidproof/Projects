...
interface IPromiseUSD {
    ...
    function makePayment(uint256 ID, uint256 amountUSD) external;
}
...
contract PromiseUSD is IERC20, IPromiseUSD {
    ...
    // Token Data
    string constant _name = "PromiseUSD";
    ...
    */
    function setApprovedContract(address Contract, bool _isApproved) external override onlyXUSD {
    ...
    */
    ...
    /**
        Repayes the debt tracked by `ID` in BUSD, and releases XUSD 
    ...
    */
    function takeLoan(uint256 ID, address desiredStable, uint256 collateral) external override onlyApproved returns (uint256) {
    ...
    */
    ...
    function _makePayment(address user, uint256 ID, uint256 amountUSD) internal returns (uint256 amountCollateral) {
        ...
        // transfer in USD
        uint256 received = _transferIn(BUSD, amountUSD);

        // Repay USD Amount To XUSD
        bool s = IERC20(BUSD).transfer(XUSD, received);
    ...
    */
    function _takeLoan(uint256 ID, address desiredStable, uint256 collateral) internal {
        ...
        uint256 received = IXSurge(XUSD).requestPromiseTokens(desiredStable, xReceived);
        require(
            received > 0 && 
    ...
    */
    function mint(uint256 amount) external override onlyXUSD returns (bool) {
