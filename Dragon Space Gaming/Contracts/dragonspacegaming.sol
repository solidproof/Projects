...
contract DragonSpace is ERC20Detailed, Ownable {
    ...
    uint256 public rewardYield = 8800;
    uint256 public rewardYieldDenominator = 10000000000;
    uint256 public maxSellTransactionAmount = 25000 * 10 ** 18;

    uint256 public rebaseFrequency = 3;
    uint256 public nextRebase = block.timestamp + 3;
    ...
    uint256 private constant MAX_REBASE_FREQUENCY = 3;
    ...
    address public liquidityReceiver = 0x4489ABaCA81aeb67A9c5C013a30Bf844CB1B1fF3;
    address public treasuryReceiver = 0x835a5981D4315711C576C3C7A4cF651508Bfb3fc;
    address public riskFreeValueReceiver = 0xf807C9aB7927f6A929C2f5C31c53FB7115425535;
    address public firePit = 0xf10bC67Fc03e9506f61535f20d4f9013d111487a;
    ...
    uint256 public buyFeeRFV = 25;
    uint256 public firepitFee = 15;
    uint256 public sellFeeTreasuryAdded = 0;
    uint256 public sellFeeRFVAdded = 10;
    uint256 public sellFeeFirePitAdded = 0;
    uint256 public sellFeeLiquidityAdded = 10;
    uint256 public totalBuyFee = liquidityFee.add(treasuryFee).add(buyFeeRFV).add(firepitFee);
    uint256 public totalSellFee = totalBuyFee.add(sellFeeTreasuryAdded)
    .add(sellFeeRFVAdded).add(sellFeeFirePitAdded).add(sellFeeLiquidityAdded);
    ...
    constructor() ERC20Detailed("DragonSpace Gaming and Rebase", "xDragon", uint8(DECIMALS)) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToFirepit = contractTokenBalance.mul(firepitFee.mul(2).add(sellFeeFirePitAdded)).div(realTotalFee);
        uint256 amountToTreasury = contractTokenBalance.sub(amountToLiquify).sub(amountToRFV).sub(amountToFirepit);
        ...
        if(amountToFirepit > 0) {
             _basicTransfer(msg.sender, firePit, amountToFirepit);

        }

        emit SwapBack(contractTokenBalance, amountToLiquify, amountToRFV, amountToTreasury, amountToFirepit);
    }
    ...
    function setFeeReceivers(address _liquidityReceiver, address _treasuryReceiver, address _riskFreeValueReceiver, address _firepit) external onlyOwner {
    ...
    function setFees(uint256 _liquidityFee, uint256 _riskFreeValue, uint256 _treasuryFee, uint256 _firepit, uint256 _sellFeeTreasuryAdded,uint256 _sellFeeFirepitAdded, uint256 _sellFeeLiquidityAdded, uint256 _sellFeeRFVAdded, uint256 _feeDenominator) external onlyOwner {
        require(
            _liquidityFee <= MAX_FEE_RATE &&
            _riskFreeValue <= MAX_FEE_RATE &&
            _treasuryFee <= MAX_FEE_RATE &&
            _firepit <= MAX_FEE_RATE&&
            _sellFeeTreasuryAdded <= MAX_FEE_RATE &&
            _sellFeeFirepitAdded <= MAX_FEE_RATE &&
        ...
        sellFeeFirePitAdded = _sellFeeFirepitAdded;
        totalBuyFee = liquidityFee.add(treasuryFee).add(buyFeeRFV).add(firepitFee);
    ...
    event SwapBack(uint256 contractTokenBalance,uint256 amountToLiquify,uint256 amountToRFV,uint256 amountToTreasury, uint256 amountToFirepit);
