 ...
 * @title NeutroEarn
 * @author Trader Joe
 * @notice NeutroEarn is a contract that allows NEUTRO deposits and receives wrapped ether sent by dex fees's daily
 * harvests. Users deposit NEUTRO and receive a share of what has been sent by dex fees based on their participation of

 * the total deposited NEUTRO. It is similar to a MasterChef, but we allow for claiming of different reward tokens
 ...
 */
contract NeutroEarn is Ownable, ReentrancyGuard {
    ...
    IERC20 public neutro;

    /// @dev Internal balance of NEUTRO, this gets updated on user deposits & withdrawals
    /// this allows to reward users with WETH
    uint256 public internalNEUTROBalance;
    ...
    /// @dev Info of each user that stakes NEUTRO
    mapping(address => UserInfo) private userInfo;

    /// @notice Emitted when a user deposits NEUTRO
    ...
    /// @notice Emitted when a user withdraws NEUTRO
    ...
    /// @notice Emitted when a user emergency withdraws its NEUTRO
    ...
    /**
     * @notice Initialize a new NeutroEarn contract
     * @dev This contract needs to receive an ERC20 `_rewardToken` in order to distribute them
     * (with MoneyMaker in our case)
     * @param _rewardToken The address of the ERC20 reward token
     * @param _neutro The address of the NEUTRO token
    ...
    constructor(
        ...
        require(
            ...
            "NeutroEarn: reward token can't be address(0)"
        );
        require(
            address(_neutro) != address(0),
            "NeutroEarn: neutro can't be address(0)"
        );
        require(
            _feeCollector != address(0),
            "NeutroEarn: fee collector can't be address(0)"
        );
        require(
            _depositFeePercent <= 5e17,
            "NeutroEarn: max deposit fee can't be greater than 50%"
        );
    ...
    /**
     * @notice Deposit NEUTRO for reward token allocation
     * @param _amount The amount of NEUTRO to deposit
     */
    function deposit(uint256 _amount) external nonReentrant {
        ...
        internalNEUTROBalance = internalNEUTROBalance.add(_amountMinusFee);
        if (_fee > 0) {
            neutro.safeTransferFrom(_msgSender(), feeCollector, _fee);
        }

        neutro.safeTransferFrom(_msgSender(), address(this), _amountMinusFee);
     ...
     * @return The amount of NEUTRO user has deposited
    ...
    function addRewardToken(IERC20 _rewardToken) external onlyOwner {
        require(
            ...
            "NeutroEarn: token can't be added"
        );
        require(rewardTokens.length < 25, "NeutroEarn: list of token too big");
        rewardTokens.push(_rewardToken);
        isRewardToken[_rewardToken] = true;
    ...
    function removeRewardToken(IERC20 _rewardToken) external onlyOwner {
        require(
            ...
            "NeutroEarn: token can't be removed"
        );
        updateReward(_rewardToken);
    ...
    function setDepositFeePercent(
        ...
        require(
            ...
            "NeutroEarn: deposit fee can't be greater than 10%"
        );
    ...
    function pendingReward(
        ...
        require(isRewardToken[_token], "NeutroEarn: wrong reward token");
        UserInfo storage user = userInfo[_user];
        uint256 _totalxNEUTRO = internalNEUTROBalance;
        uint256 _accRewardTokenPerShare = accRewardPerShare[_token];

        uint256 _currRewardBalance = _token.balanceOf(address(this));
        uint256 _rewardBalance = _token == neutro
            ? _currRewardBalance.sub(_totalxNEUTRO)
            : _currRewardBalance;

        if (_rewardBalance != lastRewardBalance[_token] && _totalxNEUTRO != 0) {
    ...
    /**
     * @notice Withdraw NEUTRO and harvest the rewards
     * @param _amount The amount of NEUTRO to withdraw
     */
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        require(
            ...
            "NeutroEarn: withdraw amount exceeds balance"
        );
        ...
        if (_amount > 0) {
            ...
            internalNEUTROBalance = internalNEUTROBalance.sub(_amount);
            neutro.safeTransfer(_msgSender(), _amount);
    ...
    function updateReward(IERC20 _token) public {
        ...
        // Did NeutroEarn receive any token
        if (_rewardBalance == lastRewardBalance[_token] || _totalxNEUTRO == 0) {
            return;
        }
        ...
        accRewardPerShare[_token] = accRewardPerShare[_token].add(
            _accruedReward.mul(ACC_REWARD_PER_SHARE_PRECISION).div(_totalxNEUTRO)
        );
    ...
    function safeTokenTransfer(
        ...
        uint256 _currRewardBalance = _token.balanceOf(address(this));
        uint256 _rewardBalance = _token == neutro
            ? _currRewardBalance.sub(internalNEUTROBalance)
            : _currRewardBalance;
