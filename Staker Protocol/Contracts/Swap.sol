...
contract Swap is BEP20, Whitelist {
    ...
    // address => first swap tx time
    mapping (address => uint256) public firstSellTimes;

    // user => (periodIndex => amount)
    mapping (address => mapping (uint256 => uint256) ) public sellPeriodAmounts;
    ...
    function setTrackingInterval(uint256 _newTrackingInterval_) public onlyOwner {
        trackingInterval_ = _newTrackingInterval_;
    }
    ...
    function tokenToBnbSwapInput(uint256 tokens_sold, uint256 min_bnb) public isNotPaused returns (uint256) {
        ...
        if(firstSellTimes[msg.sender] != 0){
          periodIndex = (block.timestamp - firstSellTimes[msg.sender]).div(periodSellLimitation);
        }else{
          firstSellTimes[msg.sender] = block.timestamp;
        }
        sellPeriodAmounts[msg.sender][periodIndex] += tokens_sold;
        require(sellPeriodAmounts[msg.sender][periodIndex] <= maxTokensPeriodicSell, "24hrs limit");
