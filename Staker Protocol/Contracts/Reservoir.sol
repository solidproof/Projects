...
contract Reservoir is Ownable {
    ...
    uint8 public entryFee_ = 10;


    uint8 public exitFee_  = 10;

    uint8 public stakerFee = 50;

    uint8 public instantFee = 20;

    uint8 public payoutRate_ = 2;
    ...
    function updateEntryFee(uint8 newEntryFee) public onlyOwner {
        entryFee_ = newEntryFee;
    }

    function updateExitFee(uint8 newExitFee) public onlyOwner {
        exitFee_ = newExitFee;
    }

    function updateStakerFee(uint8 newStakerFee) public onlyOwner {
        stakerFee = newStakerFee;
    }

    function updateInstantFee(uint8 newInstantFee) public onlyOwner {
        instantFee = newInstantFee;
    }

    function updatePayoutRate(uint8 newPayoutRate) public onlyOwner {
        payoutRate_ = newPayoutRate;
    }

    function updateMagnitude(uint256 newMagnitude) public onlyOwner {
        magnitude = newMagnitude;
    }

    function updateMinBuy(uint256 newMinBuy) public onlyOwner {
        minBuy = newMinBuy;
    }

    function updateBalanceInterval(uint256 newBalanceInterval) public onlyOwner {
        balanceInterval = newBalanceInterval;
    }

    function updateDistributionInterval(uint256 newDistributionInterval) public onlyOwner {
        distributionInterval = newDistributionInterval;
    }

    function updateSwapAddress(address newSwapAddress) public onlyOwner {
        swapToken = IToken(newSwapAddress);
        swap = ISwap(newSwapAddress);
        swapAddress = newSwapAddress;
    }

    function updateCollateralAddress(address newCollateralAddress) public onlyOwner {
        cToken = IToken(newCollateralAddress);
        collateralAddress = newCollateralAddress;
    }
    ...
    function buy() public payable returns (uint256){
        ...
        require(msg.value >= minBuy, "min buy");
    ...
    function sweep() public returns (uint256){
        ...
        if (balanceOriginTokens >= 1e18  && tokenSupply_ > 0){
            ...
            //Add the new liquidity to staker dividends;
    ...
    /// @dev Distribute undividend in and out fees across staker pools and instant divs
    function allocateFees(uint fee) private {
        uint _share = fee.div(100);
        uint _staker = _share.mul(stakerFee);           //40 --> 50
        uint _instant = _share.mul(instantFee);     //40 --> 20
        uint _lock = fee.safeSub(_staker + _instant); //20 --> 30

        if (tokenSupply_ > 0) {
            //Apply divs
            profitPerShare_ = SafeMath.add(profitPerShare_, (_instant * magnitude) / tokenSupply_);
        }
        //Add to dividend staker pools
        dividendBalance += _staker;
    ...
    function purchaseTokens(address _customerAddress, uint256 _incomingtokens) internal returns (uint256) {
        ...
        //staker and buybacks; instant requires being called after supply is updated
        allocateFees(_undividedDividends);
