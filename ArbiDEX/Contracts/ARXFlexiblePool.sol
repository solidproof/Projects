/**
 *Submitted for verification at Arbiscan on 2023-03-14
*/
...
contract ARXFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 arxAtLastUserAction; // keeps track of arx deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // ARX token
    IARXPool public immutable arxPool; //ARX pool
    ...
    //When call arxpool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositARX(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
     ...
     * @param _arxPool: ARXPool contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_arxPool), type(uint256).max);
    }

    /**
     * @notice Deposits funds into the ARX Flexible Pool.
    ...
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    arxPool.withdrawByAmount(withdrawAmount);
        ...
        emit DepositARX(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the ARX Flexible Pool
    ...
    /**
     * @notice Withdraws from ARX Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract owner.
     */
    function emergencyWithdraw() external onlyOwner {
        require(staking, "No staking arx");
        staking = false;
        arxPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the ARX Flexible Pool
     */
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = arxPool.userInfo(address(this));
        uint256 pricePerFullShare = arxPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into ARXPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            arxPool.deposit(bal, 0);
        }
    }
}