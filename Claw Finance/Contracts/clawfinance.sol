...
// File: claw/contracts/TokenVesting.sol
...
// File: claw/contracts/ClawToken.sol
...
contract ClawToken is ERC20 {
  ...
  // total amount  : 2000m
  uint256 public constant MAX_SUPPLY = 2000 * 1e6 * 1e18;

  constructor(string memory name_, string memory symbol_) Ownable() ERC20(name_, symbol_) {
    teamLockToken = new TokenVesting(msg.sender, block.timestamp + 180 days, 0, 540 days);
    //init  fluidity  100m amount
    _mint(msg.sender, 100 * 1e6 * 1e18);
    _moveDelegates(address(0), msg.sender, 100 * 1e6 * 1e18);
    //reserved  team  amount
    _mint(address(teamLockToken), 100 * 1e6 * 1e18);
    _moveDelegates(address(0), address(teamLockToken), 100 * 1e6 * 1e18);
  }
  ...
  modifier onlyMinter() {
    ...
    require(isMinter, ": caller is not from minter");
    _;
  }
  ...
  function mint(address _to, uint256 _amount)
    ...
    onlyMinter
  {
    if (totalSupply() + _amount > MAX_SUPPLY) return;
    _mint(_to, _amount);
  ...
  {
    _burn(_from, _amount);
    _moveDelegates(_from, address(0), _amount);
  }
