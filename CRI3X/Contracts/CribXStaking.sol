...
// File: contracts/4_Cri3xFarm.sol
...
contract Cri3xFarm is Ownable {
    ...
    address PROJECT_WALLET = 0x6bE6e3E53157385251814B2C0cD4aD1a26e43B0D;
    ...
    // Deposit tokens to Cri3xFarm for reward allocation.
    ...
    function depositAirdrop(uint256 _pid, address[] calldata lockFor, uint256[] calldata _amount, uint256[] calldata _lock_period) public onlyOwner {

        for(uint256 i = 0; i < _amount.length; i++) {

            PoolInfo storage pool = poolInfo[_pid];
            UserInfo storage user = userInfo[_pid][lockFor[i]];
            updatePool(_pid);
            ...
            // user first deposit
                ...
                // set lock period + boost based on it
                user.lockUntil = _lock_period[i];
                // boost is linearly dependent on lock period
                user.boost = BOOST_BASE + (((max_boost - BOOST_BASE) * _lock_period[i]) / MAX_LOCK_PERIOD);
            }

            if (_amount[i] > 0) {
                // measure delta for deflationary tokens
                uint256 balance_before = pool.depositToken.balanceOf(address(this));
                pool.depositToken.safeTransferFrom(address(msg.sender), address(this), _amount[i]);
               // _amount = pool.depositToken.balanceOf(address(this)) - balance_before;
                ...
                if (user.amount > 0) {
                    uint256 remaining_lock = user.lockUntil <= block.timestamp ? 0 : (user.lockUntil - block.timestamp);
                    // update user lock period and boost with weighted average
                    uint256 new_lock_period = (remaining_lock * user.amount + _lock_period[i] * _amount[i]) / (user.amount + _amount[i]);
                    // set lock period + boost based on it
                    // user.lockUntil = block.timestamp + new_lock_period;
                    // boost is linearly dependent on lock period
                    user.boost = BOOST_BASE + (((max_boost - BOOST_BASE) * new_lock_period) / MAX_LOCK_PERIOD);
                ...
                pool.depositedAmount += _amount[i];
                depositedTokens[address(pool.depositToken)] += _amount[i];
            }
            user.lastRewardAt = block.timestamp;
            user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
            emit Deposit(lockFor[i], _pid, _amount[i], user.lockUntil);
        }
    }
    // Deposit tokens to Cri3xFarm for reward allocation for someone else
    function lockForSomeone(uint256 _pid, address lockFor, uint256 _amount, uint256 _lock_period) public onlyOwner {
        require (_lock_period <= MAX_LOCK_PERIOD, "Bad lock period");
        require ((_amount > 0) || (_amount == 0 && _lock_period == 0), "Bad input on deposit");

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][lockFor];
        updatePool(_pid);
    ...
    // Withdraw LP tokens from Cri3xFarm.
    ...
    function emergencyUnlock(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (_amount > 0) {
            ...
            pool.depositToken.safeTransfer(address(msg.sender), (_amount / 100 * 75));
            pool.depositToken.safeTransfer(PROJECT_WALLET, (_amount / 100 * 25));
        }
        user.lastRewardAt = block.timestamp;
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
        user.lockUntil = 0;
