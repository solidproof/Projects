...
contract soulsOfMeta is Context, IERC20, Ownable {
    ...
    string private constant _name = "soulsOfMeta";
    ...
    Taxes public sellTaxes = Taxes(0,1,1,1,1);
    ...
    function setTaxes(uint256 _rfi, uint256 _staking, uint256 _community , uint256 _liquidity, uint256 _burn) public onlyOwner {
        require(_rfi + _staking + _liquidity + _community + _burn <= 12, "Fees must be lower than 12%");
        taxes.rfi = _rfi;
    ...
    function setBuyTaxes(uint256 _rfi, uint256 _staking, uint256 _community , uint256 _liquidity, uint256 _burn) public onlyOwner {
        require(_rfi + _staking + _community + _liquidity + _burn <= 12, "Fees must be lower than 12%");
        buyTaxes.rfi = _rfi;
    ...
    function setSellTaxes(uint256 _rfi, uint256 _staking, uint256 _community , uint256 _liquidity, uint256 _burn) public onlyOwner {
        require(_rfi + _staking + _liquidity + _burn <= 12, "Fees must be lower than 12%");
        sellTaxes.rfi = _rfi;
    ...
    function _getValues(uint256 tAmount, bool takeFee, uint8 category) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rStaking,to_return.rCommunity,to_return.rLiquidity, to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, uint8 category) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tStaking-s.tCommunity-s.tLiquidity-s.tBurn;
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rStaking,uint256 rCommunity , uint256 rLiquidity, uint256 rBurn) {
