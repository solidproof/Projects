 ...
 *
 *  - Preminted initial supply of 5.8 Million tokens allocated to owner
 *  - 2.8 Million to be preminted for the initial circulating supply
 *  - 1.0 Million to be preminted for the Liquidity (Split 50/50 between bsc/iotex)
 *  - 2.0 Million to be preminted for the Marketing tokens ( conductive of airdrip/iotex) and unlock schedule
 *  - Ability for holders to transfer their tokens
 *  - Ability for holders to burn (destroy) their tokens
 *  - Max supply has been set to 100 million
 *
 *
...
contract IotexPad is ERC20Burnable, Ownable {
    using SafeMath for uint256;

    uint256 private constant initialSupply = 5.8 * 10**18 * 10**6; // 5.8 million
    uint256 private constant maxSupply = 100.0 * 10**18 * 10**6; // 100 million

    constructor() ERC20("IotexPad", "TEX") {
        /**
         * @dev Mints an initial supply of 5.8 Million TEX tokens and
    ...
    function mintToken(uint256 amount) public virtual onlyOwner {
        ...
        // Check limit with max supply
        require(
            (totalSupply.add(mintAmount)) <= maxSupply,
