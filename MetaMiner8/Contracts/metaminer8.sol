...
*
* 3% Auto Reflections Rewarded in MMR
* 3% Auto Liquidity
...
contract MetaMiner is ERC20, ERC20Burnable, Ownable {
    using SafeMath for uint256;
    ...
    MetaMinerDividendTracker public dividendTracker;
    ...
    mapping(address => bool) public isAdminForDepositProfit;
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "MMR::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }

    constructor() ERC20("TetaMiner", "TMR") {

        dividendTracker = new MetaMinerDividendTracker(BUSD);
        ...
        */
        ...
        isAdminForDepositProfit[owner()] = true;
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MMR: The dividend tracker already has that address");

        MetaMinerDividendTracker newDividendTracker = MetaMinerDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MMR: The new dividend tracker must be owned by the MMR token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MMR: The router already has that address");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOwner {
        require(_maxTransferAmountRate <= 10000, "MMR::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MMR: Account is already the value of 'excluded'");
    ...
    /**
     * @notice set the account to deposit profit of BUSD from BTC mining
     */
    function setAdminForDepositProfit(address account, bool enable) external onlyOwner{
        isAdminForDepositProfit[account] = enable;
    }

    /**
     * @notice deposite the amount of BUSD from BTC mining
     */
    function depositProfitfromBTC(uint256 amount) external {
        require(amount > 0, "Wrong value");
        require(isAdminForDepositProfit[msg.sender], "Unauthorized sender");

        bool success = IERC20(BUSD).transferFrom(msg.sender, address(dividendTracker), amount);

        if (success) {
            dividendTracker.distributeCAKEDividends(amount);

            emit SendDividends(0, amount);
        }
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MMR: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MMR: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 100000 && newValue <= 500000, "MMR: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MMR: Cannot update gasForProcessing to same value");
...
contract MetaMinerDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address _cake) DividendPayingToken("Kunt_Dividend_Tracker", "Kunt_Dividend_Tracker", _cake) {
        claimWait = 1 hours;
        minimumTokenBalanceForDividends = 10_000 * (10**9); //must hold 10000 + tokens
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "Kunt_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MMR contract.");
    }
