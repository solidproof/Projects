...
This is the official staking contract of the LastApeStanding token.
The staking contract is able to create staking pools with varying:
- Interest rate
- Maturity date
- Penalty on withdraw before maturity
- Max staking amount per user
- Max users
...
contract LastApeStandingStakes is Ownable, ReentrancyGuard {
    ...
    event StakingFeaturesChanged(
        uint256 interestRate,
        uint256 maturity,
        uint256 penaltyPct
    );
    event MaxStakingAmountChanged(uint256 minStakingAmount);
    event MinStakingAmountChanged(uint256 maxStakingAmount);
    ...
    constructor(
        ...
        require(
            ...
            "Maturity date needs to be set in the future"
        );
        mainToken = _token;
        interestRate = _rate;
        maturity = _maturity;
    ...
    function stake(uint256 stakeAmount) external {
        ...
        require(
            ...
            "Staking amount is greater than maximum allowed"
        );
    ...
    function unstake(uint256 recordIndex) external nonReentrant {
        ...
        if (
            mainToken.allowance(owner(), address(this)) >= _interest &&
            mainToken.balanceOf(owner()) >= _interest
        ) {
            mainToken.transferFrom(owner(), staker, _interest);
        } else {
            _interest = 0;
        }
    ...
    function emergencyWithdraw(uint256 recordIndex) external nonReentrant {
        ...
        if (penaltyPct == 0) {
            ...
            mainToken.transfer(owner(), penalty);
        } else {
    ...
    function setFeatures(
        ...
        emit StakingFeaturesChanged(interestRate, maturity, penaltyPct);
    }

    function setMinStakingAmount(uint256 amount) public onlyOwner {
        minStakingAmount = amount;

        emit MinStakingAmountChanged(minStakingAmount);
    }

    function setMaxStakingAmount(uint256 amount) public onlyOwner {
        maxStakingAmount = amount;

        emit MaxStakingAmountChanged(maxStakingAmount);
    }
