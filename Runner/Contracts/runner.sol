...
contract Token is IERC20, Ownable {
    ...
    address private _lpAddress =
        address(0xB25CBA8EbFbA56aEa79C1e23B20441142B2347Bf);//lp
    address private _devAddress =
        address(0x461648aD355f6932F9e8AAcE1C4eD552bF4867E1);//develop
    address private _marketAddress =
        address(0x5dFc2777360B5e394835A628158b2D78D51D0748);//marketing
    ...
    function swapAndSendToFee(uint256 tokens) private lockTheSwap{
        ...
        uint256 lps = initialBalance.mul(1).div(6);
        uint256 devs = initialBalance.mul(2).div(6);
        uint256 markets = initialBalance.sub(lps).sub(devs);
        (bool success,) = address(_lpAddress).call{value: lps}("");
        if(success) {
            (bool success2,) = address(_devAddress).call{value: devs}("");
            if(success2) {
                (bool success3,) = address(_marketAddress).call{value: markets}("");
                 if(success3) {
    ...
    function _transferStandard(
        ...
        _taketaxFee(sender, tAmount.div(100).mul(_taxFee), currentRate);
