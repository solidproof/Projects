...
contract Retrogression is Context, ERC20, Ownable {
    ...
    constructor() ERC20 ("Retrogression", "RTGN") {
        ...
        _buyProductionFee = 12;
        _buyMarketingFee = 2;

        _sellProductionFee = 12;
        _sellMarketingFee = 2;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Retrogression: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != marketingWallet && to != marketingWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
            ...
            uint256 productionAmount = address(this).balance.mul(_sellProductionFee).div(totalSellFees);
    ...
    function updateBuyFees(uint256 newBuyProductionFee, uint256 newBuyMarketingFee) public onlyOwner {
        require(newBuyProductionFee.add(newBuyMarketingFee) <= 25, "Total buy taxes cannot exceed 25%");
    ...
    function updateSellFees(uint256 newSellProductionFee, uint256 newSellMarketingFee) public onlyOwner {
        require(newSellProductionFee.add(newSellMarketingFee) <= 25, "Total sell taxes cannot exceed 25%");
