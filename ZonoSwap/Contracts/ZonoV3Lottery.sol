...
// File: contracts/interfaces/IZonoV3Lottery.sol

pragma solidity ^0.8.4;

interface IZonoV3Lottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in ZONOV3 per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in ZONOV3 token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInZonoV3: price of a ticket in ZONOV3
...
// File: contracts/ZonoV3Lottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title ZonoV3 Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract ZonoV3Lottery is ReentrancyGuard, IZonoV3Lottery, Ownable {
    ...
    uint256 public maxPriceTicketInZonoV3 = 50 ether;
    uint256 public minPriceTicketInZonoV3 = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInZonoV3;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] zonov3PerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInZonoV3;
        uint32 finalNumber;
    }
     ...
     * @param _zonov3TokenAddress: address of the ZONOV3 token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _zonov3TokenAddress, address _randomGeneratorAddress) {
        zonov3Token = IERC20(_zonov3TokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of ZONOV3 to this contract
        uint256 amountZonoV3ToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInZonoV3,
            _ticketNumbers.length
        );

        // Transfer ZONOv3 tokens to this contract
        uint256 balanceBefore = zonov3Token.balanceOf(address(this));
        zonov3Token.safeTransferFrom(
            address(msg.sender),
            address(this),
            amountZonoV3ToTransfer
        );
        amountZonoV3ToTransfer = zonov3Token.balanceOf(address(this)).sub(
            balanceBefore
        );

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInZonoV3 = _lotteries[_lotteryId]
            .amountCollectedInZonoV3
            .add(amountZonoV3ToTransfer);
    ...
    function claimTickets(
        ...
        // Initializes the rewardInZonoV3ToTransfer
        uint256 rewardInZonoV3ToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInZonoV3ToTransfer = rewardInZonoV3ToTransfer.add(
                rewardForTicketId
            );
        }

        // Transfer money to msg.sender
        zonov3Token.safeTransfer(msg.sender, rewardInZonoV3ToTransfer);

        emit TicketsClaim(
            msg.sender,
            rewardInZonoV3ToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        // Calculate prizes in ZONOV for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].zonov3PerBracket[j] =
                ...
                // A. No ZONOV3 to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].zonov3PerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury = amountToWithdrawToTreasury
            .add(_lotteries[_lotteryId].amountCollectedInZonoV3)
            .sub(amountToShareToWinners);

        // Transfer ZONOV3 to treasury address
        if (amountToWithdrawToTreasury > 0) {
            zonov3Token.safeTransfer(
                treasuryAddress,
        ...
        onlyOwnerOrInjector
    {
        ...
        _amount = zonov3Token.balanceOf(address(this)).sub(balanceBefore);
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInZonoV3 >= minPriceTicketInZonoV3) &&
                (_priceTicketInZonoV3 <= maxPriceTicketInZonoV3),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInZonoV3: _priceTicketInZonoV3,
            ...
            amountCollectedInZonoV3: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)
        ...
        onlyOwner
    {
        require(_tokenAddress != address(zonov3Token), "Cannot be ZONOV3 token");
    ...
    /**
     * @notice Set ZONOV3 price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInZonoV3: minimum price of a ticket in ZONOV3
     * @param _maxPriceTicketInZonoV3: maximum price of a ticket in ZONOV3
     */
    function setMinAndMaxTicketPriceInZonoV3(
        uint256 _minPriceTicketInZonoV3,
        uint256 _maxPriceTicketInZonoV3
    ) external onlyOwner {
        require(
            _minPriceTicketInZonoV3 <= _maxPriceTicketInZonoV3,
            "minPrice must be < maxPrice"
        );

        minPriceTicketInZonoV3 = _minPriceTicketInZonoV3;
        maxPriceTicketInZonoV3 = _maxPriceTicketInZonoV3;
    }
     ...
     * @param _priceTicket price of a ticket (in ZONOV3)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].zonov3PerBracket[_bracket];
        } else {
            return 0;
        }
    }
