...
abstract contract ERC20 is Context, Ownable2Step, IERC20, IERC20Metadata, IERC20Errors {
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        unchecked {
            if (automatedMarketMakerPairs[to]) {
                ...
                feeAmount = value.mul(totalSellFee).div(feeDenominator);
                require(feeAmount <= value, "Fee amount exceeds value");

                if (feeAmount > 0) {
                _update(from, owner(), feeAmount);
                }
            }
        }
        _update(from, to, value.sub(feeAmount));
    }
...
contract BASEPADToken is ERC20 {
    constructor() ERC20("BASEPAD", "BPAD") {
