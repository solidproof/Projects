...
contract ReflectionLiquidityToken is IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    //timestamp from which no more transfer fees will apply
    uint256 public noFeeTime = 1720828800;  // TODO set timestamp to 3 month after launch date! 
    // (current date: Sat Jul 13 2024 00:00:00 GMT+0000)
    ...
    //fee going to Reflections
    uint256 public _taxFee = 100;
    uint256 private _previousTaxFee;

    //fee going to token contract for Liquidity
    uint256 public _liquidityFee = 100;
    uint256 private _previousLiquidityFee;

    //fee going to merketing wallet
    uint256 public _marketingFee = 300;
    uint256 private _previousMarketingFee;

    IUniswapV2Router02 public uniswapV2Router = IUniswapV2Router02(0x4752ba5DBc23f44D87826276BF6Fd6b1C372aD24); // uinV2 from UNISWAP on BASE mainnet
    address public uniswapV2Pair;
    address public _marketingAddress = 0x5323906f33d555228234f407157495EB73aD55Fc;  // TODO, set correct marketing addr!
    ...
    function _getRValues( uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 currentRate ) private pure 
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        //if the no-Fee-Timestamp has passed, no mpre fee will be applied to any transfer
        if (block.timestamp > noFeeTime) {
            takeFee = false;
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            address(0xdead),  // TODO set LIQ receipient addr!
            block.timestamp
        );
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee ) private {
        //if takeFee is false, then remove all fees!
