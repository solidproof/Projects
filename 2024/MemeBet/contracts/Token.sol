...
contract Token is ERC20, ERC20Burnable, Ownable {
  ...
  constructor() ERC20('Memebet', 'MEMEBET') {
    _approve(_msgSender(), _router, type(uint256).max);
    _mint(_msgSender(), 2_000_000_000 * 10 ** 18);
  }

  function enableTrading() external onlyOwner {
    require(!tradingEnabled, 'MEMEBET: trading already enabled');
    tradingEnabled = true;
    emit TradingEnabled();
  }
  ...
  function setPairs(address[] calldata pairs, bool[] calldata status) external onlyOwner {
    require(!tradingEnabled, 'MEMEBET: trading already enabled');
    require(pairs.length == status.length, 'MEMEBET: invalid parameters');
  ...
  function _transfer(address from, address to, uint256 amount) internal override {
    require(from != address(0), 'MEMEBET: transfer from the zero address');
    require(to != address(0), 'MEMEBET: transfer to the zero address');

    if (!tradingEnabled) {
      if (_pairs[from] || _pairs[to]) {
        _pairs[from] ? require(to == owner() || to == _launcher, 'MEMEBET: trading disabled') : require(from == owner() || from == _launcher, 'MEMEBET: trading disabled');
      }
    }
