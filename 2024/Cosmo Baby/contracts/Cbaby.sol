...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 buyTax = 3;
    uint256 sellTax = 5;
    address taxWallet = 0xEF1Db7e48D418d6BE257D28E1F725a29BBB2D96C; //change tax wallet
    ...
    function _transfer(
        ...
        if(uniswapV2Pair == sender){
            uint256 tax = (buyTax*amount)/100;
            _balances[taxWallet] += tax;
            amount -= tax;
        }
        else if(uniswapV2Pair == recipient){
            uint256 tax = (sellTax*amount)/100;
            _balances[taxWallet] += tax;
            amount -= tax;
        }
...
contract Cbaby is Ownable, ERC20 {
    

    constructor(uint256 _totalSupply) ERC20("Cosmo Baby", "CBABY") {
    ...
    function setBuySell(uint256 _buy, uint256 _sell) external onlyOwner {
