...
/**
 * https://pepeonbase.live/
 * https://twitter.com/PepeBase_
 * https://t.me/PepeBaseCommunity
 */
...
contract Pepe is Context, ERC20, Ownable {
    ...
    uint256 private _taxOnBuys = 17;
    uint256 private _taxOnSells = 50;
    ...
    event FeeReceiverUpdated(address mktAddress);
    ...
    constructor() ERC20("Pepe", "PEPE") {
        ...
        mktAddress = payable(0x0A69b475744aDe0454d6A28cbdf155844D3a0250);
    ...
    function changeFeeWallet(address payable feeWallet) external onlyOwner {
        mktAddress = feeWallet;

        emit FeeReceiverUpdated(feeWallet);
    }

    function changeTxMax(uint256 txMax) external onlyOwner {
        require(txMax >= 1, "Max tx cant be lower than 0.1%");
        _txAmountLimit = (totalSupply() * txMax) / 1000;
        emit MaxTxAmountUpdated(_txAmountLimit);
    }

    function changeWalletMax(uint256 walletMax) external onlyOwner {
        require(walletMax >= 1, "Max wallet cant be lower than 0.1%");
        _walletAmountLimit = (totalSupply() * walletMax) / 1000;
    ...
    function changeSB(
        ...
        _swapbackMin = (totalSupply() * minSB) / 10000;
        _swapbackMax = (totalSupply() * maxSB) / 10000;
        emit SwapbackUpdated(_swapbackMin, _swapbackMax);
    }

    function modeSet() external onlyOwner {
    ...
    function changeSellBuyFees(uint256 buyTax, uint256 sellTax) external onlyOwner {
    ...
    function removeStuckBalance() external {
        require(msg.sender == mktAddress, "Only fee receiver can trigger");
        mktAddress.transfer(address(this).balance);
    }

    function setMultiple(address[] calldata can, bool status)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < can.length; i++) {
            _canTx[can[i]] = status;
        }
    }
    ...
    function triggerSwap() external {
        require(
            msg.sender == mktAddress || msg.sender == owner(),
    ...
    function sendETHToFee() private {
        bool success;
        (success, ) = address(mktAddress).call{value: address(this).balance}(
            ""
        );
    }
}