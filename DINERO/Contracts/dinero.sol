...
contract DINERO is ApprovedEngine {
    constructor(
        address _interact,
        address _sir
    ) public payable ApprovedEngine(_interact) {
    ...
    function approveTo(address New) external ifSir {
        _approveTo(New);
    }
...
contract DINEROtoken is ERC20, Ownable {
    uint8 private constant DECIMALS = 18;
    uint256 private constant INITIAL_TOTAL_SUPPLY = 400 * (10**6) * (10 ** uint256(DECIMALS));
    ...
    function initialize() public initializer {
        ...
        ERC20.__ERC20_init("Dinero", "DINERO");
    ...
    uint16 public transferTaxRate = 9;
    uint16 public burnRate = 5;
    uint16 public treasuryRate = 3;
    uint16 public constant MAXIMUM_TRANSFER_TAX_RATE = 15;
    address public constant BURN_ADDRESS = 0xda50f316dd27818121EDe70E067091B697a2A941;
    address public constant TREASURY_ADDRESS = 0x4bC7532df77220B56eeA5a87C715b34Dbc7adeb5;

    // Max transfer amount rate in basis points. Default 1%
    ...
    event TreasuryRateUpdated(address indexed owner, uint256 previousRate, uint256 newRate);
    ...
    event UniSwapRouterUpdated(address indexed owner, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "DINERO::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || recipient == TREASURY_ADDRESS || transferTaxRate == 0) {
            super._transfer(sender, recipient, amount);
        } else {

            uint256 taxAmount = amount.mul(transferTaxRate).div(100);
            uint256 burnAmount = amount.mul(burnRate).div(100);
            uint256 treasuryAmount = amount.mul(treasuryRate).div(100);
            uint256 liquidityAmount = taxAmount.sub(burnAmount).sub(treasuryAmount);
            require(taxAmount == burnAmount + liquidityAmount + treasuryAmount, "DINERO::transfer: Burn value invalid");

            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "DINERO::transfer: Tax value invalid");

            super._transfer(sender, BURN_ADDRESS, burnAmount);
            super._transfer(sender, TREASURY_ADDRESS, treasuryAmount);
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOwner {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "DINERO::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
        emit TransferTaxRateUpdated(msg.sender, transferTaxRate, _transferTaxRate);
        transferTaxRate = _transferTaxRate;
    }

    function updateBurnRate(uint16 _burnRate) public onlyOwner {
        require(_burnRate <= 10, "DINERO::updateBurnRate: Burn rate must not exceed the maximum rate.");
        emit BurnRateUpdated(msg.sender, burnRate, _burnRate);
        burnRate = _burnRate;
    }

    function updateTreasuryRate(uint16 _treasuryRate) public onlyOwner {
        require(_treasuryRate <= 10, "DINERO::updateBurnRate: Treasury rate must not exceed the maximum rate.");
        emit TreasuryRateUpdated(msg.sender, treasuryRate, _treasuryRate);
