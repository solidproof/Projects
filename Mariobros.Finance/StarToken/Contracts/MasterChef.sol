...
contract MushroomCoin is ERC20 {
     ...
     */
    constructor(address _to, uint256 _initialSupply, uint256 _maxSupply) ERC20("Star Token", "STAR") {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. STARs to distribute per second.
        uint256 lastRewardSecond; // Last second that STARs distribution occurs.
    ...
    // The COIN!
    MushroomCoin public immutable star;
    // Dev address.
    address public devaddr;
    // COIN tokens created per second.
    uint256 public StarPerSecond;
    // Deposit Fee address
    ...
    // The block timestamp when star mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 StarPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    function initializePools(address _nativeLP) external onlyOwner {
        ...
        add(200, IERC20(_nativeLP), 0, true, false);                                        // STAR LP
        add(150, IERC20(star), 0, true, false);                                             // STAR
        ...
        add(10, IERC20(0x82f0B8B456c1A451378467398982d4834b6829c1), 400, false, false);     // MIM pool
        add(5, IERC20(0x6c021Ae822BEa943b2E66552bDe1D2696a53fbB7), 400, false, false);      // TOMB pool
        add(1, IERC20(0xB4559681514867B6Db1e7e805118Cfad16949D37), 0, true, false);         // COIN L1 pool
    }
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 coinReward = multiplier
                ...
                uint256 totalRewards = star.totalSupply().add(devReward).add(
                coinReward
            );
                if (totalRewards > star.maxSupply()) {
                coinReward = star.maxSupply().sub(star.totalSupply());
            }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= star.maxSupply()) {
            ...
            star.mint(devaddr, coinReward.div(10));
            star.mint(address(this), coinReward);
        } else {
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accStarPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeStarTransfer(msg.sender, pending);
    ...
    // Safe star transfer function, just in case if rounding error causes pool to not have enough coins.
    ...
    function updateEmissionRate(uint256 _StarPerSecond) external onlyOwner {
        require(_StarPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        StarPerSecond = _StarPerSecond;
        emit UpdateEmissionRate(msg.sender, _StarPerSecond);
    }
